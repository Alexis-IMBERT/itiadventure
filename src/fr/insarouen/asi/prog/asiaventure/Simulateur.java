...
public class Simulateur {
		...
		while (sc.hasNextLine()) {
			...
			switch (nomType) {
				...
				case "ConditionDeFinVivantDansPieceEtPossedeObjets":
					...
					this.conditionDeFins
							.add(new ConditionDeFinVivantDansPieceEtPossedeObjets(etatDuJeu, vivant, piece, lesObjets));
					break;
				default:
	...
	public EtatDuJeu executerUnTour(Scanner sc) throws java.lang.Throwable {
		...
		// v´erifier chaque condition de fin et retourner ENCOURS si aucune n’est
		// vérifiée, sinon retourner l’état
		EtatDuJeu resultat = EtatDuJeu.ENCOURS;
		for (ConditionDeFin conditionDeFin : this.conditionDeFins) {
			resultat = conditionDeFin.verifierCondition();
			if (resultat != EtatDuJeu.ENCOURS) {
				return resultat;
			}
		}
		return resultat;
